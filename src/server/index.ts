...
app.all('/mcp', (req, res) => { if (!mcpAuthorized(req)) return mcpUnauthorized(res, req); mcpDebug('handler:dispatch', { method: req.method, contentType: req.get('content-type'), accept: req.get('accept'), bodyKeys: req.body && typeof req.body === 'object' ? Object.keys(req.body) : undefined }); try { const result = mcpHandler(req, res, req.body); if (result && typeof (result as Promise<unknown>).then === 'function') { void (result as Promise<unknown>).then(() => mcpDebug('handler:complete', { method: req.method })).catch(error => mcpDebug('handler:error', { method: req.method, error: String(error instanceof Error ? error.stack ?? error.message : error) })); } } catch (error) { mcpDebug('handler:throw', { method: req.method, error: String(error instanceof Error ? error.stack ?? error.message : error) }); throw error; } });
