...
if (!MCP_TOKEN || !AGENT_TOKEN) throw new Error('MCP_TOKEN and AGENT_TOKEN must be set');
...
function buildMcpServer() {
  ...
  }, async () => resultContent(registry.list()));
  ...
  server.registerTool('delete_file', {
    description: 'Delete a file or empty directory on a connected Windows computer.',
    inputSchema: z.object({ agentId: agentIdSchema, path: z.string().min(1) }),
  }, async ({ agentId, path }) => resultContent(await registry.call(agentId, 'delete_file', { path })));

  server.registerTool('execute_powershell', {
    description: 'Execute a PowerShell command on a connected Windows computer. Disabled by default on the agent.',
