...
function buildMcpServer() {
  ...
  // MCP v2's registerTool has two input-schema representations. Keep the dynamic
  // coding-tool helper compatible with the Zod version used by this project while
  // preserving the concrete schemas at each call site.
  const codingTool = (name: ToolName, description: string, shape: any) =>
    server.registerTool(name, { description, inputSchema: shape }, async (args: Record<string, unknown>) =>
  ...
  server.registerTool('execute_powershell', { description: 'Execute PowerShell 7.1 on Windows. Agent policy may disable it.', inputSchema: z.object({ agentId: agentIdSchema, command: z.string().min(1) }) }, async ({ agentId, command }) => resultContent(await registry.call(agentId, 'execute_powershell', { command })));
  ...
  codingTool('run_python', 'Run Python with arguments in the workspace. By default waits for completion with COMMAND_TIMEOUT_MS; set async=true to start a background Python job and receive a jobId. Requires ALLOW_COMMAND_EXECUTION=true.', { agentId: agentIdSchema, args: z.array(z.string()).default([]), cwd: cwdSchema, async: z.boolean().default(false) });
  codingTool('python_job_inspect', 'Inspect a background Python job by jobId, including status and captured stdout/stderr.', { agentId: agentIdSchema, jobId: z.string().min(1) });
  codingTool('python_job_kill', 'Terminate a background Python job by jobId and its process tree. Requires ALLOW_COMMAND_EXECUTION=true.', { agentId: agentIdSchema, jobId: z.string().min(1) });
  codingTool('python_jobs', 'List background Python jobs known by this Windows agent.', { agentId: agentIdSchema });
