...
function buildMcpServer() {
  ...
  codingTool('git', 'Run safe Git version-control operations for repository inspection and management. Use this tool for git status, diff, log, show, blame, branch, add, commit, and other non-apply Git operations. Do NOT use it to modify file contents. NEVER use git apply or git am. NEVER use git checkout, restore, reset, or clean to overwrite or discard working-tree files unless the user explicitly requests it. For file edits, use apply_patch, edit_file, or write_file. After editing, use git diff to inspect changes. Requires ALLOW_COMMAND_EXECUTION=true.', { agentId: agentIdSchema, args: z.array(z.string()).min(1), cwd: cwdSchema }, z.object({ result: commandResultSchema }));
  codingTool('apply_patch', 'Apply a patch directly to create, update, move, or delete files in the workspace. This is the primary tool for normal source-code edits and multi-file changes. Do not invoke git apply, git am, PowerShell, Bash, or another shell command to apply the patch. After applying a patch, use git diff when repository-level verification is needed. Requires ALLOW_COMMAND_EXECUTION=true.', { agentId: agentIdSchema, patch: z.string().min(1), cwd: cwdSchema }, z.object({ result: commandResultSchema }));
