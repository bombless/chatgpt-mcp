...
class AgentRegistry {
  ...
  add(agentId: string, socket: WebSocket) {
    const old = this.agents.get(agentId); old?.close(4000, 'replaced by a newer connection');
    this.agents.set(agentId, socket);
    socket.on('close', () => { if (this.agents.get(agentId) === socket) this.agents.delete(agentId); });
  }
  ...
  async call(agentId: string, tool: ToolName, args: Record<string, unknown>) {
    const socket = this.get(agentId); if (!socket) throw new Error(`Windows agent '${agentId}' is not connected`);
    const id = crypto.randomUUID(); const request: AgentRequest = { type: 'request', id, tool, args };
    return await new Promise<unknown>((resolve, reject) => {
      const timer = setTimeout(() => { this.pending.delete(id); reject(new Error(`Agent request timed out after ${REQUEST_TIMEOUT_MS}ms`)); }, REQUEST_TIMEOUT_MS);
      this.pending.set(id, { resolve: value => { clearTimeout(timer); resolve(value); }, reject: error => { clearTimeout(timer); reject(error); } });
      try { socket.send(JSON.stringify(request)); } catch (error) { this.pending.delete(id); clearTimeout(timer); reject(error instanceof Error ? error : new Error(String(error))); }
    });
  }
  handleMessage(message: AgentMessage) {
    if (message.type !== 'response') return;
    const response = message as AgentResponse; const item = this.pending.get(response.id); if (!item) return;
    this.pending.delete(response.id); if (response.ok) item.resolve(response.result); else item.reject(new Error(response.error ?? 'Agent request failed'));
  }
}
...
function mcpUnauthorized(res: express.Response) {
  ...
  return res.status(401).json({ error: 'unauthorized', error_description: 'OAuth access token required' });
}
...
wss.on('connection', ws => {
  let agentId: string | undefined; let initialized = false;
  ws.on('message', raw => { try { const message = JSON.parse(raw.toString()) as AgentMessage; if (message.type === 'hello') { if (!/^[a-zA-Z0-9._-]{1,64}$/.test(message.agentId)) return ws.close(4002, 'invalid agentId'); agentId = message.agentId; initialized = true; registry.add(agentId, ws); console.log(`[agent] connected ${agentId} (${message.hostname})`); return; } if (!initialized) return ws.close(4003, 'hello required'); registry.handleMessage(message); } catch { ws.close(4004, 'invalid message'); } });
  ws.on('close', () => { if (agentId) console.log(`[agent] disconnected ${agentId}`); });
});
httpServer.listen(PORT, '0.0.0.0', () => { console.log(`MCP gateway listening on :${PORT}`); console.log(`Public MCP: ${PUBLIC_URL}/mcp`); console.log(`Agent endpoint: ${PUBLIC_URL}/agent`); });
