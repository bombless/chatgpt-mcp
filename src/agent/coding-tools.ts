...
async function exec(command: string, args: string[], cwd?: string, timeout = COMMAND_TIMEOUT_MS) {
  const result = await execFileAsync(command, args, { cwd: cwd ? assertAllowed(cwd) : WORKSPACE_ROOT, windowsHide: true, timeout, maxBuffer: MAX_OUTPUT_BYTES, shell: false });
...
async function command(name: 'npm' | 'python' | 'node', args: Record<string, unknown>) {
  ...
  try { return await exec(executable, commandArgs, cwd); }
...
async function rg(args: Record<string, unknown>) {
  ...
  rgArgs.push(query, '.');
  try { return { ...(await exec('rg', rgArgs, cwd)), matches: true, truncated: false }; }
  catch (error: any) {
...
async function processList() {
  if (process.platform === 'win32') return await exec('tasklist', ['/FO', 'CSV', '/NH']);
  return await exec('ps', ['-eo', 'pid,ppid,comm,args']);
}
...
async function killProcess(args: Record<string, unknown>) {
  ...
  if (process.platform === 'win32') await exec('taskkill', ['/PID', String(pid), '/T', '/F']); else await exec('kill', ['-TERM', String(pid)]);
  return { ok: true, pid };
}
...
async function git(args: Record<string, unknown>) {
  ...
  return await exec(process.platform === 'win32' ? 'git.exe' : 'git', gitArgs, cwd);
}
...
async function applyPatch(args: Record<string, unknown>) {
  ...
  try {
    ...
    return await exec(process.platform === 'win32' ? 'git.exe' : 'git', ['apply', '--whitespace=nowarn', temp], cwd);
